...
public class AlipayCore_O {
     ...
     * @param flag  日志文件的标识
     */
    public static void logResult(String sWord, String flag) {
        FileWriter writer = null;
        try {
            writer = new FileWriter(AlipayConfig.log_path + flag + System.currentTimeMillis() + ".txt");
            writer.write(sWord);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public static String createLinkStringNoSort(Map<String, String> params) {
        ...
        return ("service=" + params.get("service")) + "&v=" + params.get("v") + "&sec_id=" + params.get("sec_id") +
