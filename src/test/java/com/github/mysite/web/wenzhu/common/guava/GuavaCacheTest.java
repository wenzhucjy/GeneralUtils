...
public class GuavaCacheTest {
	...
	public void whenCacheReachMaxWeight_thenEviction() {
		...
		final LoadingCache<String, String> cache = CacheBuilder.newBuilder().maximumWeight(16).weigher(weighByLength)
				.build(loader);
		cache.getUnchecked("first");
		cache.getUnchecked("second");
		cache.getUnchecked("third");
	...
	public void whenEntryIdle_thenEviction() throws InterruptedException {
		...
		final LoadingCache<String, String> cache = CacheBuilder.newBuilder().expireAfterAccess(2, TimeUnit.MILLISECONDS)
				.build(loader);
	...
	public void whenEntryLiveTimeExpire_thenEviction() throws InterruptedException {
		...
		final LoadingCache<String, String> cache = CacheBuilder.newBuilder().expireAfterWrite(2, TimeUnit.MILLISECONDS)
				.build(loader);
		cache.getUnchecked("hello");
		assertEquals(1, cache.size());
		Thread.sleep(300);
